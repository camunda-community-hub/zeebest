use crate::{Client, ActivatedJob, JobResult, CompleteJob};
use std::pin::Pin;
use futures::{Future, FutureExt};
use std::sync::Arc;

pub trait JobCompleter {
    fn complete(&self, key: i64, variables: Option<String>) -> Pin<Box<dyn Future<Output = Result<(), crate::Error>> + Send>>;
    fn fail(&self, key: i64, retries: i32, error_message: Option<String>) -> Pin<Box<dyn Future<Output = Result<(), crate::Error>> + Send>>;
}
...
impl JobCompleter for Completer {
    fn complete(&self, key: i64, variables: Option<String>) -> Pin<Box<dyn Future<Output=Result<(), crate::Error>> + Send>> {
        let complete_job = CompleteJob { job_key: key, variables };
        self.client.complete_job(complete_job).boxed()
    }

    fn fail(&self, key: i64, retries: i32, error_message: Option<String>) -> Pin<Box<dyn Future<Output=Result<(), crate::Error>> + Send>> {
        let error_message = error_message.unwrap_or("".to_string());
        self.client.fail_job(key, retries - 1, error_message).boxed()
    }
}

pub struct JobClient {
    complete: Arc<dyn JobCompleter + Send + Sync>,
}

impl JobClient {
    pub fn new<T: JobCompleter + Send + Sync + 'static>(completer: T) -> Self {
        Self { complete: Arc::new(completer) }
    }
    ...
    pub fn report_status(&self, activated_job: ActivatedJob, job_result: JobResult) -> Pin<Box<dyn Future<Output = Result<(), crate::Error>> + Send>> {
        ...
        match job_result {
            ...
            JobResult::Complete { variables } => {
                self.complete.complete(key, variables)
            }
        }
    }
}
...
#[cfg(test)]
mod test {
    use crate::worker::job_client::JobCompleter;
    use std::pin::Pin;
    use futures::{Future, FutureExt};
    ...
    impl JobCompleter for MockCompleter {
        fn complete(&self, key: i64, _variables: Option<String>) -> Pin<Box<dyn Future<Output=Result<(), crate::Error>> + Send>> {
    ...
    fn calls_client_method_on_completed_job() {
        ...
        let job_client = JobClient { complete: Arc::new(completer.clone()) };
